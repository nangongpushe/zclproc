...
@Controller
@RequestMapping("StorageFourUnit")
public class StorageFourUnitController extends BaseController{
	 ...
	 @RequestMapping("index")
		public String index(ModelMap map) {
		 ...
		 map.put("storageWarehouseList",storageWarehouseList);
			return "/StorageFourUnit/storageStarUnit_list";
		}
		@SysLogAn("访问：仓储管理-粮库管理-四化粮库申报")
	 @RequestMapping("")
		...
		* 修改时间: 2017年9月28日 下午5:03:12
		...
		public String toAdd(HttpServletRequest request,ModelMap map){
			...
			List storageWarehouseList=storageWarehouseService.getWarehouseList();
			map.put("storageWarehouseList",storageWarehouseList);
			return "/StorageFourUnit/storageStarUnit_add";
		}
		...
		* 修改时间: 2017年9月29日 下午5:59:25
		...
		public String edit(HttpServletRequest request,ModelMap map,String id){
			...
			List storageWarehouseList=storageWarehouseService.getWarehouseList();
			map.put("storageWarehouseList",storageWarehouseList);
			return "/StorageFourUnit/storageStarUnit_edit";
		}
		...
		* 修改时间: 2017年9月29日 下午5:59:25
		...
		public String view(HttpServletRequest request,ModelMap map,String id){
			...
			map.addAttribute("sysFile", sysFile);
			return "/StorageFourUnit/storageStarUnit_view";
		}
...
//					}
					...
					sysLogService.add(request, "仓储管理-粮库管理-四化粮库申报-新增");
				}else {
					storageStarUnit.setId(id);
					storageStarUnitService.update(storageStarUnit);
					sysLogService.add(request, "仓储管理-粮库管理-四化粮库申报-修改");
				}
		...
		* 修改时间: 2017年10月3日 下午1:37:42
		...
		public String export(HttpServletRequest request, HttpServletResponse response) {
			...
			try {
				...
				list=storageStarUnitService.exportxls(request);
			} catch (Exception e) {
				e.printStackTrace();
			}
			String fileName = "星级粮库评优表.xls";
			try {
...
//			response.setCharacterEncoding("UTF-8");
			...
			String fileName = storageStarUnit.getWarehouse()+"-"+storageStarUnit.getCreateDate()+"-"+"四化粮库评优表.xls";
			try {
				fileName = java.net.URLEncoder.encode(fileName, "UTF-8");
			} catch (UnsupportedEncodingException e) {
				// TODO Auto-generated catch block
...
//			response.setCharacterEncoding("UTF-8");
			response.setHeader("Content-Disposition", "attachment;fileName="+fileName);
			request.setAttribute("exportList", list);
			return "/StorageFourUnit/storageStarUnit_exportxls";
		}
}
