...
(function(root, factory) {
	...
	Paging.prototype = {
		...
		format: function() {
			...
			if (this.current == 1) {
				$('.js-page-prev', this.container).addClass('ui-pager-disabled ui-pager-disabled-first');
				$('.js-page-first', this.container).addClass('ui-pager-disabled');
			}
			if (this.current == this.pagecount) {
				$('.js-page-next', this.container).addClass('ui-pager-disabled ui-pager-disabled-last');
		...
		bindToolbar: function() {
			var _this = this, html = '';
			html = $('<li class="ui-paging-toolbar">' +
                //'<select class="ui-select-pagesize"></select>' +
                '到第<input type="text" class="ui-paging-count"/>页<a href="javascript:void(0)">确定</a><span>共'+this.settings.count+'条</span></li>');
