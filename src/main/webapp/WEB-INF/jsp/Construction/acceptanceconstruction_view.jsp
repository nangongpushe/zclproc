...
<div id="outSide">
	...
	<div id="listArea">
		...
		<div class="buttonArea" id="add-acceptance">新增</div>
		</shiro:hasPermission>
		<table style="display:none">
			<tr id="template">
				...
				<td name="do">
					<a class='layui-btn layui-btn-primary layui-btn layui-btn-mini detail' href="${ctx }/Construction/Detail/Acceptance.shtml?id=">查看</a>
					<shiro:hasPermission name="constructionInfo:edit">
					<a class='layui-btn layui-btn-primary layui-btn layui-btn-mini edit edit' href="${ctx }/Construction/Acceptance/Edit.shtml?cid=">编辑</a>
		...
		<table class="layui-table sort-table">
			...
			<tbody id="data-list">
				<c:forEach items="${acceptancelist.result}" var="acceptance">
					<tr>
						<td align="left">${acceptance.projectSerial }</td>
						<td align="center">${acceptance.projectYear }</td>
						<td align="left">${acceptance.projectName }</td>
						<td align="left">${acceptance.projectUnit }</td>
						<td align="left"><c:if test="${not empty acceptance.parentProject}">${acceptance.parentProject.projectSerial}(${acceptance.parentProject.projectName})</c:if></td>
						<td>
							<a class='layui-btn layui-btn-primary layui-btn layui-btn-mini detail' href="${ctx }/Construction/Detail/Acceptance.shtml?id=${acceptance.id }">查看</a>
							<shiro:hasPermission name="constructionInfo:edit">
							<a class='layui-btn layui-btn-primary layui-btn layui-btn-mini edit' href="${ctx }/Construction/Acceptance/Edit.shtml?cid=${acceptance.id }">编辑</a>
							<span class='layui-btn layui-btn-primary layui-btn layui-btn-mini delete' tag="${acceptance.id }">删除</span>
							</shiro:hasPermission>
						</td>
					</tr>
				</c:forEach>
				<c:if test="${acceptancelist.result==null || fn:length(acceptancelist.result)==0}"><tr><td colspan="5" align="center">暂无数据</td></tr></c:if>
			</tbody>
		</table>
		<div id="jockerLay">
			...
			<div id="pageSelecter">
				...
				<input id="pageNumber" value="${acceptancelist.pageIndex}" style="display:inline;text-align:center;"/>
				<span style="padding-left:5px">页</span>
				<div id="page-in">确定</div>
			</div>
			<div style="display:inline;height:26px;">
				<span style="padding-right:5px">共</span><span id="total">${acceptancelist.totalCount}</span><span style="padding-left:5px">条</span>
				<select id="pageSize" style="border-radius:2px;border:1px #ccc solid;height:26px;">
					<option <c:if test="${acceptancelist.pageSize == 10}">selected</c:if> value="10">10 条/页</option>
					<option <c:if test="${acceptancelist.pageSize == 20}">selected</c:if> value="20">20 条/页</option>
					<option <c:if test="${acceptancelist.pageSize == 30}">selected</c:if> value="30">30 条/页</option>
					<option <c:if test="${acceptancelist.pageSize == 40}">selected</c:if> value="40">40 条/页</option>
					<option <c:if test="${acceptancelist.pageSize == 50}">selected</c:if> value="50">50 条/页</option>
					<option <c:if test="${acceptancelist.pageSize == 60}">selected</c:if> value="60">60 条/页</option>
					<option <c:if test="${acceptancelist.pageSize == 70}">selected</c:if> value="70">70 条/页</option>
					<option <c:if test="${acceptancelist.pageSize == 80}">selected</c:if> value="80">80 条/页</option>
					<option <c:if test="${acceptancelist.pageSize == 90}">selected</c:if> value="90">90 条/页</option>
				</select>
			</div>
		</div>
	</div>
...
<script>
	var PageIndex = Number("${acceptancelist.pageIndex}");
	var PageSize = Number("${acceptancelist.pageSize}");
	var TotalCount = Number("${acceptancelist.totalCount}");
			...
			success : function(data) {
				...
				if (data.totalCount > 0) {
					...
					for (var i = 0; i < Result.length; i++) {
                        ...
                        template.find("a").attr("data-id",Result[i].id);
						...
						$(target).find(".detail,.edit,.delete").show();
	...
	$("#add-acceptance").click(function() {
		window.location.href = "${ctx}/Construction/Acceptance/Add.shtml";
	});
    ...
    function toView(o){
        var index = layer.open({
            ...
            area: ['893px', '600px'],
            content: '${ctx }/Construction/Detail/Acceptance.shtml?id='+$(o).attr('data-id')
        });
        layer.full(index);
    }
</script>
