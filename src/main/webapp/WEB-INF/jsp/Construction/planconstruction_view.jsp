...
<div id="outSide">
		...
		<form id="select-parm" action="View.shtml" method="POST">
			<table>
				...
				<tr>
					...
					<td>
						<div>经办人：</div>
						<input id="operator" name="operator" class="inputArea"/>
					</td>
					<td>
						<div>经办时间：</div>
						<input class="inputArea" name="handleTime"/>
					</td>
				</tr>
				</thead>
			</table>
	...
	<div id="listArea">
		<div class="buttonArea" id="add-plan">新增</div>
		<table style="display:none">
			<tr id="template">
				...
				<td style="text-align: right" name="investmentSubtotal"></td>
				<td style="text-align: right" name="lastInvestmentSubtotal"></td>
				<td style="text-align: right" name="currentInvestmentSubtotal"></td>
				<td style="text-align: center" name="handleTime"></td>
				<td style="text-align: left" name="operator"></td>
				<td style="text-align: center" name="do">
					<a class='layui-btn layui-btn-primary layui-btn layui-btn-mini detail' data-id="${plan.id }" onclick="toView(this)">查看</a>
					<a style="display:none" class='layui-btn layui-btn-primary layui-btn layui-btn-mini edit' href="${ctx }/Construction/Plan/Edit.shtml?cid=${plan.id }">编辑</a>
					<span style="display:none" class='layui-btn layui-btn-primary layui-btn layui-btn-mini delete' tag="${plan.id }">删除</span>
				</td>
			</tr>
		</table>
		<table class="layui-table sort-table">
			<thead>
				<tr>
					<td style="text-align: center">年度计划名称</td>
					<td style="text-align: center">年份</td>
					<td style="text-align: center">项目单位</td>
					<td style="text-align: center">总投资小计(万元)</td>
					<td style="text-align: center">上年底累计投资额小计(万元)</td>
					<td style="text-align: center">本年计划投资额小计(万元)</td>
					<td style="text-align: center">经办时间</td>
			...
			<tbody id="data-list">
				<c:forEach items="${planlist.result }" var="plan">
					<tr>
						<td align="left"><c:if test="${plan.id != null}">${plan.year }年度基建项目投资计划表</c:if></td>
						<td align="center">${plan.year }</td>
						<td align="left">${plan.projectUnit }</td>
						<td align="right"><fmt:formatNumber value="${plan.investmentSubtotal }" maxFractionDigits="2"/></td>
						<td align="right"><fmt:formatNumber value="${plan.lastInvestmentSubtotal }" maxFractionDigits="2"/></td>
						<td align="right"><fmt:formatNumber value="${plan.currentInvestmentSubtotal }" maxFractionDigits="2"/></td>
						<td align="center"><fmt:formatDate value="${plan.handleTime }" pattern="yyyy年MM月dd日"/> </td>
						<td align="left">${plan.operator }</td>
						<td>
							<a class='layui-btn layui-btn-primary layui-btn layui-btn-mini detail' data-id="${plan.id }" onclick="toView(this)">查看</a>
							<a class='layui-btn layui-btn-primary layui-btn layui-btn-mini' href="${ctx }/Construction/Plan/Edit.shtml?cid=${plan.id }">编辑</a>
							<span class='layui-btn layui-btn-primary layui-btn layui-btn-mini delete' tag="${plan.id }">删除</span>
						</td>
					</tr>
				</c:forEach>
			</tbody>
			...
			success:function(data){
				...
				if(data.totalCount > 0){
					...
					for(var i = 0;i < Result.length; i++){
						...
						template.find("td[name='investmentSubtotal']").html(keepTwoDecimal(Result[i].investmentSubtotal));
						template.find("td[name='lastInvestmentSubtotal']").html(keepTwoDecimal(Result[i].lastInvestmentSubtotal));
						template.find("td[name='currentInvestmentSubtotal']").html(keepTwoDecimal(Result[i].currentInvestmentSubtotal));
						var time = new Date();
