...
<div id="outSide">
		...
		<form id="select-parm" action="View.shtml" method="POST">
			<table>
				...
				<tr <c:if test="${identity ne null}">style="display:none"</c:if>>
					<td>
						<div>项目单位：</div>
						<input name="projectUnit" class="inputArea" <c:if test="${identity ne null}">value="${identity }"</c:if>/>
					</td>
				</tr>
				</tbody>
			</table>
	...
	<div id="listArea">
	<shiro:hasPermission name="newConstruction:add"><div class="buttonArea" id="add-construction">新增</div></shiro:hasPermission>
		<table style="display:none">
			<tr id="template">
				<td style="text-align: left;padding-left: 1%;" name="referenceNo"><a></a></td>
				...
				<td name="do">
					...
					<a class='layui-btn layui-btn-primary layui-btn layui-btn-mini' href="${ctx }/Construction/New/Edit.shtml?cid=">编辑</a>
		...
		<table class="layui-table sort-table">
			<thead>
				<tr>
					...
					<th style="text-align: center">项目单位</th>
					<th style="text-align: center">项目名称</th>
                    <th style="text-align: center">父项目编号(名称)</th>
					<th style="text-align: center">项目年份</th>
					<th style="text-align: center">建筑类型</th>
			...
			<tbody id="data-list">
				<c:forEach items="${constructionlist.result }" var="construction">
					<tr>
						<td style="text-align: left;padding-left: 1%;">${construction.referenceNo }</td>
						<td style="text-align: left;padding-left: 1%;">${construction.projectSerial }</td>
						<td style="text-align: left;padding-left: 1%;">${construction.projectUnit }</td>
						<td style="text-align: left;padding-left: 1%;">${construction.projectName }</td>
                        <td style="text-align: left;padding-left: 1%;">
							<c:if test="${not empty construction.parentProject}">
								${construction.parentProject.projectSerial }(${construction.parentProject.projectName})
							</c:if>
						</td>
						<td style="text-align: center">${construction.projectYear }</td>
						<td style="text-align: left;padding-left: 1%;">${construction.constructionType }</td>
						<td style="text-align: left;padding-left: 1%;">${construction.projectStatus }</td>
						<td>
							<a class='layui-btn layui-btn-primary layui-btn layui-btn-mini detail' data-id="${construction.id }" onclick="toView(this)">查看</a>
							<shiro:hasPermission name="newConstruction:edit">
							<a class='layui-btn layui-btn-primary layui-btn layui-btn-mini' href="${ctx }/Construction/New/Edit.shtml?cid=${construction.id }">编辑</a>
							<span class='layui-btn layui-btn-primary layui-btn layui-btn-mini delete' tag="${construction.id }">删除</span>
							</shiro:hasPermission>
						</td>
					</tr>
				</c:forEach>
				<c:if test="${constructionlist.result==null || fn:length(constructionlist.result)==0}"><tr><td colspan="8" align="center">暂无数据</td></tr></c:if>
			</tbody>
		</table>
		<div id="jockerLay">
...
<script>
		...
		$("#select-parm").ajaxSubmit({
			...
			success:function(data){
				...
				if(data.totalCount > 0){
					...
					$("#data-list").empty();
					$("#data-list").append("<tr><td colspan='8'>搜索目标无结果</td></tr>");
					DrawPageButton("pagination");
				}
			}
		});
	...
	$("#add-construction").click(function(){
		window.location.href = "${ctx}/Construction/Add/New.shtml";
	});
	...
	function DeleteFunction(ele){
		...
		layer.confirm('你确定要'+$(self).html()+'吗？', {
			...
			var id = $(self).attr("tag");
		  	$.ajax({
		  		url:"${ctx}/Construction/Delete/Project/"+id+".shtml",
		  		type:"post",
		  		success:function(data){
		  			if(data.success){
