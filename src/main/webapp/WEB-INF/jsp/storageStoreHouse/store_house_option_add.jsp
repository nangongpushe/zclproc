...
layui.use(['form','laydate'],function() {
	...
	form.on('submit(submit_btn)',function(){
		...
		var json = JSON.stringify(form.serializeObject());
       	$.ajax({
           	type : 'POST',
           	url : '${ctx }/storageStoreHouseOption/save.shtml?auvp=${auvp }',
           	dataType : "json",
            ...
            success : function(result) {
                if(result.success) {
					layer.msg(result.msg, {icon : 1, time : 1000}, function(){
						if ('${auvp }' === 'a') {
							location.href = '${ctx }/storageStoreHouse.shtml?flag=${flag}';
						} else if ('${auvp }' === 'u'){
							location.href = '${ctx }/storageStoreHouse/editPage.shtml?id=' + '${storageStoreHouse.id }';
						}
					});
				} else {
                    layer.closeAll();
