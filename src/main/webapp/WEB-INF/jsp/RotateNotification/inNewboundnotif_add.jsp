        ...
        <div id="dataArea" class="infoArea">
            ...
            <div id="mainInfo">
                ...
                <div>
                    ...
                    <input class="inputArea" type="text" value="${notice.noticeSerial }" name="noticeSerial" maxlength="15" disabled="disabled">
                ...
                <div>
                    ...
                    <span style="text-align:right"><b class="red" id="annex" style="display: inline">*</b>附件：</span>
            ...
            <div style="display: flex;flex-direction: row;align-items: center;height: 40px">
                <c:if test="${notice.status ne '已完成' }">
                    ...
                    <div class=" layui-inline" style="display: flex;flex-direction: row;align-items: center;height: 30px;padding-top:6px;margin-left: 10px;">
                        <nobr>
                            <input id="manr" type="checkbox" name="pestCheckTypeList" value="手工填报" title="手工填报" lay-skin="primary" checked/>
                        </nobr>
                    </div>
                </c:if>
            </div>
                ...
                <tbody id="data-result">
                    ...
                    <c:forEach items="${notice.noticeDetail }" var="item">
                        <tr class="data-tr" tag="${item.dealSerial }">
                            <td style="display: none">手工填报</td>
                            <td align="left" name="variety">
                                <select>
                                    <option>请选择</option>
                                    <c:forEach items="${grainTypes}" var="grainType">
                                        <option <c:if test="${item.variety eq grainType.value}">selected</c:if> value="${grainType.value}">${grainType.value}</option>
                                    </c:forEach>
                                </select>
                            </td>
                            ...
                            <td class="handharvestYear" name="harvestYear">
                                    ${item.harvestYear }
                            </td>
                            <td name="enterpriseName">${item.enterpriseName}</td>
                            <td name="enterpriseId" style="display: none">${item.enterpriseId}</td>
                            <td align="left" name="storageWarehouse" accepteunit="${notice.storeEnterprise.id}" warehouseId="${item.warehouse.id}">
                                <select lay-filter="businessWareHouse" class="select" lay-verify="type" name="${item.dealSerial}">
                                    <option>请选择</option>
                                    <c:forEach items="${item.enterpriseWarehouses}" var="enterpriseWarehouse">
                                            <option <c:if test="${item.warehouse.warehouseShort eq enterpriseWarehouse.enterpriseWareHouseName}">selected</c:if>>${enterpriseWarehouse.enterpriseWareHouseName}</option>
                                    </c:forEach>
                                </select>
                            </td>
                            <td align="left" name="storehouse">
                                <select id= ${item.dealSerial}>
                                    <option>请选择</option>
                                    <c:forEach items="${item.enterpriseWarehouseNames}" var="wareStore">
                                        <option <c:if test="${item.storehouse eq wareStore}">selected</c:if>>${wareStore}</option>
                                    </c:forEach>
                                </select>
                            </td>
                            <td name="planNumber">
...
<script>
    ...
    $(".handharvestYear").removeAttr("lay-key");
    layui.use('laydate', function(){
        var laydate = layui.laydate
        $('.handharvestYear').each(function(){
            laydate.render({
                elem: this,
                trigger: 'click',
                type: "year",
        ...
        if (patchAll != false && temp1 == false){
            ...
            if (a0){
                $(".data-tr").each(function () {
                    ...
                    var a1 = $(this).find("td[name='storageWarehouse']>select>option:selected").html();
                    var a2 = $(this).find("td[name='variety']>select>option:selected").html();
                    var a3 = $(this).find("td[name='storehouse']>select>option:selected").html();

                    if (a1 == "请选择" || a2 == "请选择" || a3 == "请选择") {
                        alert("下拉框必须填上");
                        patchAll = false;
                        return false;
                    }
        ...
        if (temp1){
            ...
            $(".data-tr").each(function () {
                ...
                detailData.push({
                    "enterpriseId":$(this).find("td[name='enterpriseId']").html().trim(),
                    "dealSerial": $(this).attr("tag") == "" ? 0 : $(this).attr("tag"),
                    "variety": $(this).find("td[name='variety']>select>option:selected").html(),
                    "batchNumber": $(this).find("td[name='batchNumber']").html().trim(),
                    "cost": parseFloat($(this).find("td[name='cost']>input").val().trim()),
                    "pipeAttribute": $(this).find("td[name='pipeAttribute']>input").val().trim(),
                    "harvestYear": $(this).find("td[name='harvestYear']").html().trim(),
                    "storageWarehouse": $(this).find("td[name='storageWarehouse']>select>option:selected").html(),
                    "warehouse": {"id":$(this).find("td[name='storageWarehouse']").attr('warehouseId')},
                    "storehouse": $(this).find("td[name='storehouse']>select>option:selected").html(),
                    "planNumber": $(this).find("td[name='planNumber']").find("input").val(),
                    "actualNumber": $(this).find("td[name='actualNumber']").find("input").val(),
                });
            });
    ...
    layui.use('form',function () {
        var form = layui.form;
        var mReDaLi;
        form.on('select(businessWareHouse)', function(data){
            var ss = data.elem.name;
            $.ajax({
                type:'POST',
                url:'${ctx }/storageWarehouse/findStoreHouse.shtml',
                data: {'manuBatNum': data.elem.name},
                async: false,
                error: function (request) {

                },
                success: function (mReDataList) {
                    mReDaLi = mReDataList;
                }
            })
            ...
            if (data.value == "请选择"){
                ...
                for (var i = 0; i < mReDaLi.length; i++){

                    if (data.value == mReDaLi[i].storageShortName) {
                        $("#"+ ss).empty();
                        var storeList = mReDaLi[i].warehouseNumber;
                        for (var j = 0; j < storeList.length; j++){
                            $("#" + ss).append("<option>" + storeList[j] + "</option>");
                        }
                    }
                }
            }
