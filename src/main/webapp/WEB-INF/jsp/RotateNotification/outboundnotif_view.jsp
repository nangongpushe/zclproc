...
<div class="row clear-m">
	<ol class="breadcrumb">
		<li>轮换业务</li>
		<li>通知书管理</li>
		<li class="active">出库通知书</li>
	</ol>
</div>
<div id="outSide">
	<div id="userInfo" class="infoArea">
		<form id="select-parm" action="${type }/Out/List.shtml" method="POST" class="layui-form">
			<table>
				<thead>
				<tr>
					<td>
						<div style="width: 100px">通知书编号：</div>
						<div><input type="text" class="layui-input" id="scheme-name" name="noticeSerial" autocomplete="off"/></div>

					</td>
					<td>
						<div style="width: 100px">接收单位：</div>
						<div><select class="layui-select" name="storeEnterprise.id">
					...
					<td>
						<div style="width: 100px">出库截止时间：</div>
				...
				<tr>
					<td>
						...
						<div><select id="rotate-type" class="layui-select" name="status">
							<option value="" selected="selected">全部</option>
							<option>待提交</option>
							<option>驳回</option>
							<option>待审核</option>
	...
	<div id="listArea">
		...
		<table style="display:none">
			<tr id="template">
				<td align="left" name="noticeSerial"><a></a></td>
				<td align="left" name="accepteUnit"></td>
				<td name="storageDate"></td>
				<td align="left" name="documentNumber"></td>
				<td align="left" name="creater"></td>
				<td name="createtime"></td>
				<td align="left" name="status"></td>
				<td name="completeDate"></td>
				<td name="do">
					<shiro:hasPermission name="RotateNotice:edit">
					<a style="display:none" class='layui-btn layui-btn-primary layui-btn layui-btn-mini edit' href="${ctx }/RotateNotif/Out/Edit.shtml?nId=">编辑</a>
					</shiro:hasPermission>
			...
			<tbody id="data-list">
				...
				<c:forEach items="${noticelist.result }" var="notice">
					<tr <c:if test="${!notice.reportProgress}">style="background-color: rgb(255,192,203)"</c:if> <c:if test="${notice.reportProgress}">style="background-color: rgb(176,224,230)"</c:if>>
						...
						<td align="center"><fmt:formatDate value="${notice.storageDate }" pattern="yyyy年MM月dd日" /></td>
						<td align="left">${notice.documentNumber }</td>
						<td align="left">${notice.creater }</td>
						<td align="center"><fmt:formatDate value="${notice.createTime }" pattern="yyyy年MM月dd日" /></td>
						...
						<c:choose>
							<c:when test="${notice.status == '待提交' or notice.status == '驳回'}">
								<shiro:hasPermission name="RotateNotice:edit">
								<a class='layui-btn layui-btn-primary layui-btn layui-btn-mini edit' href="${ctx }/RotateNotif/Out/Edit.shtml?nId=${notice.id }">编辑</a>
								</shiro:hasPermission>
...
<script>
			...
			success:function(data){
				...
				if(data.totalCount > 0){
					...
					for(var i = 0;i < Result.length; i++){
                       ...
                       /* var time1 = new Date();
						time1.setTime(Result[i].completeDate);
						//alert(time.getFullYear())
						template.find("td[name='completeDate']").html(Result[i].completeDate!=null?time1.getFullYear()+"年"+((time1.getMonth()+1)<10?"0"+(time1.getMonth()+1):(time1.getMonth()+1))+"月"+(time1.getDate()<10?'0'+time1.getDate():time1.getDate())+"日":"———");*/
                        var time_s1 =null;
                       if(Result[i].completeDate!=null) {
                           time_s1 = Result[i].completeDate.split('-');
					   }
                        template.find("td[name='completeDate']").html(Result[i].completeDate!=null?time_s1[0]+"年"+time_s1[1]+"月"+time_s1[2]+"日":"———");
	...
	$("#add-inboundnotif").click(function(){
		window.location.href = "${ctx}/RotateNotif/Out/Add.shtml";
	});
	...
	function updateStatus(data){
        $("#simple-form").ajaxSubmit({
            url:"${ctx}/RotateNotif/Out/Edit.shtml",
            data:data,
            type:"post",
            success:function(data){
                if(data.success){
