...
<div class="row clear-m">
	<ol class="breadcrumb">
		<li>轮换业务</li>
		<li>通知书管理</li>
		<li class="active">移库通知书</li>
	</ol>
</div>
<div id="outSide">
	<div id="userInfo" class="infoArea">
		<form id="select-parm" action="${type }/Move/List.shtml" method="POST" class="layui-form">
			...
			<table>
				...
				<tr>
					...
					<td>
						...
						<div>
							<input id="complete-date" name="completeDate" class="layui-input" autocomplete="off"/>
						</div>
					</td>
					<td></td>
				</tr>
		...
		<table style="display:none">
			<tr id="template">
				...
				<td name="do">
					...
					<a style="display:none" class='layui-btn layui-btn-primary layui-btn layui-btn-mini edit' href="${ctx }/RotateNotif/Move/Edit.shtml?nId=">编辑</a>
					</shiro:hasPermission>
					...
					<tr>
						...
						<c:choose>
							<c:when test="${notice.status == '待提交' or notice.status == '驳回'}">
								<shiro:hasPermission name="RotateNotice:edit">
								<a class='layui-btn layui-btn-primary layui-btn layui-btn-mini edit' href="${ctx }/RotateNotif/Move/Edit.shtml?nId=${notice.id }">编辑</a>
								</shiro:hasPermission>
...
<script>
	...
	$("#add-inboundnotif").click(function(){
		window.location.href = "${ctx}/RotateNotif/Move/Add.shtml";
	});
	...
	function updateStatus(data){
        $("#simple-form").ajaxSubmit({
            url:"${ctx }/RotateNotif/Move/Edit.shtml",
            data:data,
            type:"post",
            success:function(data){
